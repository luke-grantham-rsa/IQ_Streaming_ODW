import math
import sys
from rsxdwstreaming import xdw_streaming, adw, pdw_basic, pdw_expert, ctrl_xdw, xdw_payload, xdw_extension, odw
...
class TabsWidget(QWidget):
    def __init__(self, parent):
        ...
        self.tabs.addTab(self.tab_odw, "ODW")
...
class AdwTab(QWidget):
    ...
    def initWidgetAdw(self):
        ...
        self.l_seg_interrupt = QLabel("Segment interruptable: ")
        self.cb_seg_interrupt = QCheckBox()
        ...
        layout.addRow(self.l_burstPRI, self.le_burstPRI)
        layout.addRow(self.l_seg_interrupt, self.cb_seg_interrupt)
    ...
    def createXdw(self):
        ...
        bSegInterrupt = int(self.cb_seg_interrupt.isChecked())
        new_pdw = adw.Adw(seg_interrupt=bSegInterrupt, payload=xdw_payload.XdwPayloadSegmentArb(segment),
                          level_offset=level_offset, freq_offset=freq_offset, phase_offset=phase_offset)

        if self.cb_extension.isChecked():
            new_pdw.extensions[0] = xdw_extension.XdwExtensionBurst(
    ...
    def createXdw(self):
        ...
        # real-time Pulse
            ...
            # linear chirp
                ...
                new_pdw = pdw_basic.PdwBasic(toa=toa,
                                             payload=xdw_payload.PdwPayloadRtChirpLinear(t_on=pw, freq_step=freq_inc),
                                             level_offset=level_offset, freq_offset=freq_offset,
                                             phase_offset=phase_offset)
            ...
            # triangular chirp
                ...
                freq_inc = (2 * self.atof(self.le_freqdev)) / (n_samples - 1)
                new_pdw = pdw_basic.PdwBasic(toa=toa,
                                             payload=xdw_payload.PdwPayloadRtChirpTriangular(t_on=pw, freq_step=freq_inc),
                                             level_offset=level_offset, freq_offset=freq_offset,
                                             phase_offset=phase_offset)
            # barker
...
class OdwTab(QWidget):
    def __init__(self, parent=None):
        super(OdwTab, self).__init__(parent)
